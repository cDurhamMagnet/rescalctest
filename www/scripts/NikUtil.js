...
MEUtil.IS_TOUCH_DEVICE = (function( userAgent ) {
    if (userAgent.match(/Android/i)
        ...
        || userAgent.match(/ZuneWP7/i)) return true;
    else return false;
})(navigator.userAgent);
...
/*---------- Public Methods ----------*/
MEUtil.addClass = function( elm, className ) { elm.className += ' ' + className; };
...
MEUtil.on = function( element, types, callback ) {
    ...
    for (var i = 0, l = typesArr.length; i < l; i++) element.addEventListener(typesArr[i], callback, false);
};

MEUtil.off = function( element, types, callback ) {
    var typesArr = types.split(' ');
    for (var i = 0, l = typesArr.length; i < l; i++) element.removeEventListener(typesArr[i], callback, false);
};

MEUtil.upscaleCanvas = function( can, ratio ) {
    if (can.substring) can = document.getElementById(can);
    if (!ratio) ratio = MEUtil.PIXEL_RATIO;
    var w = can.clientWidth || can.width, h = can.clientHeight || can.height, ctx = can.getContext('2d');
...
MEUtil.loadFromDataSrc = function( assets, options ) {
    ...
    for (var i = 0, l = count; i < l; i++) {
        var item = assets[i];
        if (typeof item != 'string') loadAssetFromDataSrc(item);
        else loadAssetFromPath(item);
    }
    ...
    function loadAssetFromPath( path ) {
        ...
        xmlReq.onreadystatechange = function() {
            if (xmlReq.readyState == 4 && xmlReq.status == 200) //getImgLoadedFunction returns a function.
...
MEUtil.ImageStackr = function( spriteContainer ) {
    var _animSpeed = 0
        ...
        //Public Methods
        getAnimationSpeed = function() { return _animSpeed; }
        , getCanvas = function() { return _canvas; }
        , getContext = function() { return _canvasCtx; }
        , getCurrentFrame = function() { return _currentFrame; }
        , getElement = function() { return _spriteContainer; }
        , getFrame = function() { return _currentFrame; }
        , getId = function() { return _spriteId; }
        ,

        //In place for use with TweenLite.
        getProgress = function() { return ((_currentFrame - 1) / (_imageCount - 1)); }
        , gotoFrame = function( newFrame ) {
              newFrame = (newFrame + 0.5) >> 0;
              if (newFrame != _currentFrame)
                  if (newFrame >= 1 && newFrame <= _imageCount) {
                      var canvasWidth = _canvas.width, canvasHeight = _canvas.height, frame = _frames[newFrame - 1];
        ...
        loadSpriteSheet = function( img, count, width, height, initFrame ) {
            var i = 0, hasMultipleSprites, hasMultipleCounts;
            ...
            try {
                ...
                try {
                    ...
                    for (i = 0; i < count.length; i++) _imageCount += count[i];
            ...
            _canvas.style.width = _stackrCanvas.style.width =
                _spriteContainer.clientWidth ? _spriteContainer.clientWidth + 'px' : _frameWidth + 'px';
            _canvas.style.height = _stackrCanvas.style.height =
                _spriteContainer.clientHeight ? _spriteContainer.clientHeight + 'px' : _frameHeight + 'px';

            _canvasCtx = _canvas.getContext('2d');
            _stackrCanvasCtx = _stackrCanvas.getContext('2d');

            //Setup _frames array
            var frame = {}, tempFrames = [];
            if (hasMultipleSprites) {
                var numImgs = 0, cols = 0;
            ...
            if (initFrame === 1) _frames = tempFrames;
            else {
                for (i = initFrame - 1; i < tempFrames.length; i++) _frames.push(tempFrames[i]);
                for (i = 0; i < initFrame - 1; i++) _frames.push(tempFrames[i]);
            }
        ...
        , nextFrame = function() { gotoFrame(_currentFrame < _imageCount ? _currentFrame + 1 : 1); }
        ...
        , previousFrame = function() { gotoFrame(_currentFrame > 1 ? _currentFrame - 1 : _imageCount); }
        ...
        , setFrame = function( newFrame ) { gotoFrame(newFrame); }
        , setProgress = function( val ) { progress(val); }
...
MEUtil.raf = (function() {
    ...
    return function( callback ) { requestAnimationFrame(callback); };
})();

MEUtil.drawLine = function( ctx, arr, startColor, nextColor ) {
    var sColor = startColor ? startColor : '#000000', nColor = nextColor ? nextColor : '#FFFFFF';
...
MEUtil.setPrefixedEvent = function( el, evtType, callback ) {
    ...
    for (var i = 0, l = prefixes.length; i < l; i++) el.addEventListener(prefixes[i] + evtType, callback, false);
    el.addEventListener(evtType.toLowerCase(), callback, false);
};

MEUtil.removePrefixedEvent = function( el, evtType, callback ) {
    var prefixes = MEUtil.PREFIXES;
    for (var i = 0, l = prefixes.length; i < l; i++) el.removeEventListener(prefixes[i] + evtType, callback, false);
    el.removeEventListener(evtType.toLowerCase(), callback, false);
};

MEUtil.getPrefixedProp = function( prop ) {
    if (MEUtil.vendorPrefix[prop]) return MEUtil.vendorPrefix[prop];
    else {
        var prefixes = MEUtil.PREFIXES
        ...
        if (lowProp in el.style) ref[prop] = lowProp;
        else {
            var preProp = '';
            ref[prop] = null;
...
MEUtil.setRuleProperty = function( selector, prop, val ) {
    var sheets = document.styleSheets, rules = null, ii = 0, l = 0;
...
function NikSplash( callback, imagePaths, option_ ) {
    if (!(this instanceof NikSplash)) return new NikSplash(callback, imagePaths, option_);
    ...
    if (option.disable) {
        ...
        (function() {
            ...
            for (var i = 0, l = pathElements.length; i < l; i++) allAssets.push(pathElements[i]);
            if (imagePaths) {
                l = imagePaths.length;
                for (i = 0; i < l; i++) allAssets.push(imagePaths[i]);
            }
        })();

        if (allAssets.length) MEUtil.loadFromDataSrc(allAssets);
        // Compile all assets to load.
    ...
    if (MEUtil.IS_TOUCH_DEVICE)
        tut.addEventListener('touchmove'
            , function( event ) {
                if (event.type === 'touchmove') if (event.touches.length === 1) event.preventDefault();
            }
            , false);
    ...
    function init() {
        ...
        (function() {
            ...
            var inset = 10, border_radius = 14, lineW = 2;
            ...
            ctx.arcTo(pxAdj(canvas.width) - inset
                , pxAdj(canvas.height) - lineW
                , pxAdj(canvas.width) / 2
                , pxAdj(canvas.height) - lineW
                , border_radius);
            ...
            grd = ctx.createRadialGradient(pxAdj(canvas.width) / 2
                , -10
                , 0
                , pxAdj(canvas.width) / 2
                , -pxAdj(canvas.width) / 3
                , pxAdj(canvas.width) / 1.5);
            ...
            function pxAdj( widthOrHeight ) { return widthOrHeight / MEUtil.PIXEL_RATIO; }
        })();
        ...
        (function() {
            ...
            img.src = '/html5/Nikon/includes/images/nikonLogoAll.png';
            ...
            function animate( alpha ) {
                ...
                alpha -= 1 / 30;
                if (alpha > 0.0001) MEUtil.raf(function() { animate(alpha); });
            }
            ...
            function drawLogoImg( img, yPad ) {
                ...
                var x = (pxAdj(canvas.width) - convertedWidth) / 2, y = (pxAdj(canvas.height) - convertedHeight) / 2;
        ...
        var rootPath = '/html5/Nikon/includes/images/';
    ...
    function startPreload() {
        ...
        MEUtil.raf(function loadLoop() {
            if (clipW < w) MEUtil.raf(loadLoop);
            else setTimeout(function() { MEUtil.raf(reverseSplashAnimation); }, 500);
            if (clipW < currentW) {
                if (clipW <= currentW - inc) clipW += inc;
                else clipW = currentW;
        ...
        if (allAssets.length)
            MEUtil.loadFromDataSrc(allAssets
                , {
                    onProgress: function( e ) { currentW = e.progress * w; }
                });
        else currentW = w;
    }
    ...
    if (id) {
        ...
        if (MEUtil.IS_TOUCH_DEVICE)
            this._container.addEventListener('touchstart'
                ...
                , false);
    } else console.error(id + ' is not a valid id string or HTMLElement.');
}
...
NikComponent.prototype.addEventListener = function( type, listener ) {
    ...
    if (handlers)
        if (typeof handlers === 'function') this[type] = [handlers, listener];
        else this[type].push(listener);
    else this[type] = [listener];
};

NikComponent.prototype.getElement = function() { return this._container; };

NikComponent.prototype.getEnabled = function() { return this._isEnabled; };

NikComponent.prototype.setEnabled = function( isEnabled ) {
    if (isEnabled != this._isEnabled)
        if (isEnabled) this._enable();
        else this._disable();
...
NikComponent.prototype._dispatchEvent = function( type ) {
    ...
    if (typeof handlers === 'function') handlers.call(this, new NikEvent(type, this));
    else if (handlers)
        for (var i = 0, l = handlers.length; i < l; i++) handlers[i].call(this, new NikEvent(type, this));
};
...
function NikSlider( id, options ) {
    ...
    this.dynamicPercentage = options.dynamicPercentage;
    ...
    this._eventListener = MEUtil.IS_TOUCH_DEVICE ? this._touchHandler.bind(this) : this._mouseHandler.bind(this);
    this._handle = {
    ...
    this._orient = (function() {
        if (!options.orientation || options.orientation.toLowerCase() == 'horizontal') return 'width';
        return 'height';
    })();
    this._spectrum = {
        index: 0
    ...
    NikSlider.prototype._setWidthHeight = function() {
    ...
    // Allows Percentage widths on the Slider with resize
    if(this.dynamicPercentage){
        var that = this;
        $(window).resize(function() {
            that._setWidthHeight();
            that.setPosition(that._handle.position, false, true);
        });
    }
    ...
    if (this._isSpectrum) this._initSpectrum(options);

    this._container.addEventListener(MEUtil.IS_TOUCH_DEVICE ? 'touchstart' : 'mousedown', this._eventListener, false);
...
NikSlider.prototype.getMinThreshold = function() { return this._thresh.max; };

NikSlider.prototype.getMaxThreshold = function() { return this._thresh.min; };
...
NikSlider.prototype.getPosition = function( min, max ) {
    ...
    if (arguments.length) {
        if (max === undefined) return pos * min;
...
NikSlider.prototype.getSpectrumIndex = function( percent ) {
    var min = this._spectrum.min, max = this._spectrum.max, p = percent == undefined ? this._handle.position : percent;

    return Math.round(min + ((max - min) * p));
};

NikSlider.prototype.getSpectrumRGB = function( percent ) {
    var index = this.getSpectrumIndex(percent), c = [];
    ...
    } else if (index <= 700) c[0] = 255;
    else c[0] = 255 - (((index - 700) / 80) * 155);
...
NikSlider.prototype.getSpectrumHex = function( percent ) {
    var rgb = this.getSpectrumRGB(percent), hexArr = [];
    hex = '#', i;

    for (i = 0; i < 10; i++) hexArr[i] = i;
    hexArr = hexArr.concat(['a', 'b', 'c', 'd', 'e', 'f']);

    for (var i = 0; i < rgb.length; i++) hex += hexArr[Math.floor(rgb[i] / 16)] + '' + hexArr[(rgb[i] % 16)];
...
NikSlider.prototype.setMinThreshold = function( min ) {
    ...
    } else console.error('The min must be a valid number.');
};
...
NikSlider.prototype.setMaxThreshold = function( max ) {
    ...
    } else console.error('The max must be a valid number.');
};

NikSlider.prototype.setPosition = function( newPos, shouldTween, doNotCallOnChange ) {
    ...
    if (typeof newPos === 'number') {
        ...
        if (shouldTween) this._setTween(true);
        this._prepareHandleTranslation(newPos, doNotCallOnChange);
    } else
        console.error(typeof newPos
            + '('
            + newPos
...
NikSlider.prototype._attachClasses = function() {
    ...
    if (this._orient == 'height') $(this._container).addClass(css.vertical);
...
NikSlider.prototype._constrainPosition = function( pos ) {
    ...
    pos = pos < thresh.min ? thresh.min : pos > thresh.max ? thresh.max : pos;
...
NikSlider.prototype._containerPt = function() {
    if (this._orient == 'height') return MEUtil.getGlobalY(this._container) - this._pointerOffset();
    else return MEUtil.getGlobalX(this._container) - this._pointerOffset();
};

NikSlider.prototype._containerX = function() { return MEUtil.getGlobalX(this._container) - this._pointerOffset(); };

NikSlider.prototype._containerY = function() { return MEUtil.getGlobalY(this._container) - this._pointerOffset(); };

NikSlider.prototype._createSpectrumCanvii = function() {
    this._handle.spectrumCan = MEUtil.createCanvas(
        this._handle.innerWidth
        , this._handle.innerHeight);
...
NikSlider.prototype._displayLoop = function() {
    ...
    if (this._isSpectrum) this._updateSpectrumHandle();
};

NikSlider.prototype._getLocal = function( touchPt ) { return touchPt - this._containerPt(); };

NikSlider.prototype._getLocalX = function( touchPt ) { return touchPt - this._containerX(); };

NikSlider.prototype._getLocalY = function( touchPt ) { return touchPt - this._containerY(); };
...
NikSlider.prototype._getThreshMinX = function( min ) { return min * this._track[this._orient]; };

NikSlider.prototype._getThreshMaxX = function( max ) { return max * this._track[this._orient]; };

NikSlider.prototype._handleTouchStart = function( touchPt ) {
    if (!this._tween) this._setTween(true);
    this._isActive = true;
...
NikSlider.prototype._handleTouchMove = function( touchPt ) {
    if (this._tween && this.tickCount < 2) this._setTween(false);
    this._setPosition(this._pxToPos(this._getLocal(touchPt)));
};
...
NikSlider.prototype._prepareHandleTranslation = function( pos, doNotCallOnChange ) {
    this._handle.position = pos;
    if (this._orient == 'height') this._handle.y = this._posToPx(pos);
    else this._handle.x = this._posToPx(pos);
...
NikSlider.prototype._setPosition = function( newPos, shouldTween ) {
    ...
    if (newPos != this._handle.position) this.setPosition(newPos);
};
...
NikSlider.prototype._setTween = function( shouldTween ) {
    if (this._tween !== shouldTween) {
        $(this._handle.border)[shouldTween ? 'addClass' : 'removeClass']('sliderHandleTween');
    ...
    switch (event.type) {
        ...
        case 'mousedown':
            if (event.which === 1 && this._isEnabled) {
                this._container.removeEventListener('mousedown', this._eventListener);
                MEUtil.on(document, 'mousemove mouseup mouseout', this._eventListener);
        ...
        case 'mouseout': // Fall-through (if on body)
            ...
            if (style === 'dashed') break;
        case 'mouseup':
            MEUtil.off(document, 'mousemove mouseup mouseout', this._eventListener);
            this._container.addEventListener('mousedown', this._eventListener);
...
NikSlider.prototype._touchHandler = function( event ) {
    ...
    switch (event.type) {
        ...
        case 'touchstart':
            if (this._isEnabled) {
                this._container.removeEventListener('touchstart', this._eventListener);
                MEUtil.on(document, 'touchmove touchend touchcancel', this._eventListener);
        ...
        case 'touchend':
            ...
            for (var i = 0, l = touches.length; i < l; i++)
                if (this._touch.identifier === touches[i].identifier) {
                    MEUtil.off(document, 'touchmove touchend touchcancel', this._eventListener);
                    this._container.addEventListener('touchstart', this._eventListener);
...
NikSlider.prototype._updateSpectrumHandle = function() {
    var ctx = this._handle.spectrumCan.getContext('2d'), pix = this.getSpectrumRGB(this._handle.position);
...
NikSlider.prototype._updateSpectrumTrack = function() {
    var ctx = this._track.spectrumCan.getContext('2d'), pix = [], w = this._track.width, h = this._track.height;
...
NikSlider.prototype._updateThreshold = function() {
    if (!this._thresh.updating) {
        ...
        var update = function() {
            ...
            if (this._orient == 'height')
                thresh.element.style.clip = 'rect('
                    + Math.round(this._getThreshMaxX(thresh.max))
                    + 'px 0 '
                    + Math.round(this._getThreshMinX(thresh.min))
                    + 'px '
                    + thresh.element.clientWidth
                    + 'px)';
            else
                thresh.element.style.clip = 'rect(0 '
                    + Math.round(this._getThreshMaxX(thresh.max))
                    + 'px '
                    + thresh.element.clientHeight
                    + 'px '
                    + Math.round(this._getThreshMinX(thresh.min))
                    + 'px)';
...
function NikButton( id, isToggle ) {
    //Kills the need for 'new' operator.
    if (!(this instanceof arguments.callee)) return new arguments.callee(id, isToggle);
    ...
    if (isToggle) {
        ...
        crater.style[MEUtil.getPrefixedProp('Transform')] = 'translate(5px, ' + ((h - 14) / 2) + 'px)';
    ...
    this._container.addEventListener(MEUtil.IS_TOUCH_DEVICE ? 'touchstart' : 'mouseover', this._eventListener);
}

MEUtil.makeSuper(NikButton, NikComponent);

NikButton.prototype.getToggleState = function() { return this._isToggledOn; };

NikButton.prototype.setToggleState = function( isToggledOn, shouldCallEvent ) {
    this._setToggle(isToggledOn);

    if (shouldCallEvent) if (this.ontouch) MEUtil.raf(this.ontouch.bind(this));
};
...
NikButton.prototype._touchHandler = function( event ) {
    ...
    switch (event.type) {
        ...
        case 'touchend':
            ...
            for (var i = 0, l = touches.length; i < l; i++)
                if (this._touch.identifier === touches[i].identifier) {
                    MEUtil.off(container, 'touchend touchcancel', this._eventListener);
...
NikButton.prototype._setToggle = function( newState ) {
    ...
    if (this.toggleText)
        if (this.toggleText.length > 1) this.text.innerHTML = this.toggleText[+newState];
        else this.text.innerHTML = this.toggleText[0];
};

NikButton.prototype._setStateUp = function() {
    $(this._container).removeClass('buttonDown');
    if (this._isToggle) this._setToggle(!this._isToggledOn);
...
function NikRadioGroup( id, options ) {
    ...
    this._orientation = options.orientation && options.orientation.toLowerCase() || 'horizontal';
...
NikRadioGroup.prototype.getButtonAtIndex = function( index ) { return this._btns[index] || null; };

NikRadioGroup.prototype.getLength = function() { return this._btns.length; };

NikRadioGroup.prototype.getSelectedButton = function() { return this._selectedBtn || null; };
...
NikRadioGroup.prototype.setBorderRadius = function( newRad, backRad ) {
    if (!isNaN(newRad)) {
        ...
        MEUtil.setRuleProperty('.' + css.horizontal + ':first-child'
            , 'border-radius'
            , newRad + 'px 0 0 ' + newRad + 'px');
        MEUtil.setRuleProperty('.' + css.horizontal + ':last-child'
            , 'border-radius'
            , '0 ' + newRad + 'px ' + newRad + 'px 0');
        MEUtil
            .setRuleProperty('.' + css.vertical + ':first-child', 'border-radius', newRad + 'px ' + newRad + 'px 0 0');
        MEUtil.setRuleProperty('.' + css.vertical + ':last-child'
            , 'border-radius'
            , '0 0 ' + newRad + 'px ' + newRad + 'px');
...
NikRadioGroup.prototype.setButtonStyleProperty = function( propName, valArr ) {
    var btns = this._btns, l = btns.length, btn = null;
    if (typeof propName === 'string' && typeof valArr === 'object' && valArr.length && valArr.length === l
    ) for (var i = 0; i < l; i++) btns[i].style.setProperty(propName, valArr[i], null);
    else console.error('Style property: ' + propName + ' was unable to be added with the values: ' + valArr);
};

NikRadioGroup.prototype.setButtonTitle = function( index, title ) {
    var btns = this._btns;
    if (typeof index === 'number' && title !== undefined)
        if (index >= 0 && index < btns.length) btns[parseInt(index, 10)].children[0].innerHTML = title;
    else console.error(index + ' and ' + title + ' are not valid arguments.  Pass a number and string.');
};

NikRadioGroup.prototype.setCSSConfiguration = function( cssConfig ) {
    var rules = this._config.CSS;
    for (var prop in cssConfig)
        if (rules[prop]) rules[prop] = cssConfig[prop];
        else console.error(prop + ' is not a configuration property.');
    this._updateButtons();
};

NikRadioGroup.prototype.setCustomProperty = function( propName, valArr ) {
    var btns = this._btns, l = btns.length;
    if (typeof propName === 'string' && typeof valArr === 'object' && valArr.length && valArr.length === l
    ) for (var i = 0; i < l; i++) btns[i][propName] = valArr[i];
    else console.error('Custom property: ' + propName + ' was unable to be added with the values: ' + valArr);
};

NikRadioGroup.prototype.setGutterSize = function( newSize ) {
    newSize = typeof newSize === 'number' ? newSize + 'px' : newSize;

    MEUtil.setRuleProperty('.' + this._config.CSS.vertical + ':nth-child(n+2)', 'margin', newSize + ' 0 0 0');
    MEUtil.setRuleProperty('.' + this._config.CSS.horizontal + ':nth-child(n+2)', 'margin', '0 0 0 ' + newSize);
};
...
NikRadioGroup.prototype.setSelectedIndex = function( newSelection, shouldDispatch ) {
    if (isNaN(newSelection)) {
        ...
        this._selectedBtn = null;
    } else if (!this._selectedBtn || newSelection !== this._selectedBtn.haRadioIndex) {
        var btns = this._btns
            , btn = btns[newSelection < -1
                  ? 0
                  : newSelection >= btns.length ? btns.length - 1 : parseInt(newSelection, 10)];
...
NikRadioGroup.prototype.setTitles = function( newTitles ) {
    if (newTitles && newTitles.length) {
        ...
        for (var i = 0, l = btns.length; i < l; i++)
            btns[i].children[0].innerHTML = newTitles[i] || btns[i].children[0].innerHTML;
    } else console.error('setTitles: ' + newTitles + ' - value passed was not a valid array.');
};

NikRadioGroup.prototype
    ._addClass = function( element, className ) { $(element).addClass(this._config.CSS[className]); };
...
NikRadioGroup.prototype._addTouchEndListener = function( btn ) {
    var onTypes = MEUtil.IS_TOUCH_DEVICE ? 'touchend touchcancel' : 'mouseup mouseout'
...
NikRadioGroup.prototype._createButtonArray = function( srcArr ) {
    ...
    for (var i = 0, l = srcArr.length; i < l; i++) {
        ...
        if (srcArr[i].className) btn.userClassName = srcArr[i].className;
        btns[i] = btn;
    }
    return btns;
};
...
NikRadioGroup.prototype._handleBtnPressed = function( btn, shouldDispatch ) {
    ...
    if (shouldDispatch) this._dispatchEvent('touchstart');
};
...
NikRadioGroup.prototype._handleBtnReleased = function( btn, shouldDispatch ) {
    ...
    if (btn) {
        ...
        if (this._touches && this._touches.length) /*If there are multiple touches*/
            this._unselect(btn);
        else {
            if (selectedBtn) this._unselect(selectedBtn);
            this._selectedBtn = btn;
            if (shouldDispatch) this._dispatchEvent('change');
        }
    }
};
...
NikRadioGroup.prototype._removeTouchEndListener = function( btn ) {
    var eTypes = MEUtil.IS_TOUCH_DEVICE ? 'touchend touchcancel' : 'mouseup mouseout';
...
NikRadioGroup.prototype._touchHandler = function( event ) {
    ...
    switch (event.type) {
        case 'touchstart':
            if (this._isEnabled) this._handleBtnPressed(btn, true);
            break;
        case 'touchcancel': //Fall-through
        case 'touchend':
            this._handleBtnReleased(btn, true);
...
NikLoader.prototype.show = function() { this._container.style.opacity = 1; };

NikLoader.prototype.hide = function() { this._container.style.opacity = 0; };